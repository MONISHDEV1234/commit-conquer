...
function CartProvider({ children }) {
  const [state, dispatch] = useReducer(cartReducer, { items: [], isOpen: false }, (init) => {
    try {
      const local = localStorage.getItem("store_cart");
      return local ? JSON.parse(local) : init;
    } catch {
      return init;
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem("store_cart", JSON.stringify({ items: state.items, isOpen: false }));
  }, [state.items]);
